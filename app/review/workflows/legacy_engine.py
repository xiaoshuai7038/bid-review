...
from app.ai.providers.opencode.client import OpenCodeClient
...
def _completion_tail_page_tolerance(total_pages: int) -> int:
    raw = (os.getenv("BID_REVIEW_COMPLETION_TAIL_PAGE_TOLERANCE") or "").strip()
    if raw:
        try:
            value = max(0, int(raw))
            return value
...
def _evaluate_review_completion(
    ...
    *,
    ...
    tender_required_matches = 0
    tender_actual_matches = 0
    if tender_expected_sections:
        tender_required_matches = min(len(tender_expected_sections), max(3, len(tender_expected_sections) // 2))
        tender_actual_matches = _count_scope_matches(tender_expected_sections, tender_reviewed_sections)
        if tender_actual_matches < tender_required_matches:
            reasons.append(
                f"招标文件关键章节覆盖不足，仅覆盖 {tender_actual_matches}/{len(tender_expected_sections)} 个关键块。"
            )
    if tender_total_pages and tender_pages_seen < tender_total_pages:
        shortfall = tender_total_pages - tender_pages_seen
        tolerance = _completion_tail_page_tolerance(tender_total_pages)
        allow_tail_gap = bool(tender_expected_sections) and tender_actual_matches >= tender_required_matches and shortfall <= tolerance
